...
you pass the [getter form TanStack recommends](https://tanstack.com/query/v5/docs/framework/vue/reactivity)
directly, with no `computed` wrapper:

```ts
// All three forms work:
useShowPetById(petId.value); // plain value
useShowPetById(petId); // Ref
useShowPetById(() => props.petId); // getter - idiomatic for reactive props
```
